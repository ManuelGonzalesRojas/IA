...
% Regla para obtener la cantidad total de hijos, varones y mujeres de una persona
cantidad_hijos_y_genero(Persona, Total, Varones, Mujeres) :-
    findall(Hijo, (progenitor(Persona, Hijo), varon(Hijo)), VaronesLista),
    findall(Hija, (progenitor(Persona, Hija), mujer(Hija)), MujeresLista),
    append(VaronesLista, MujeresLista, Hijos),
    length(Hijos, Total),
    length(VaronesLista, Varones),
    length(MujeresLista, Mujeres).

% Regla para obtener el porcentaje de varones y mujeres entre los hijos de una persona
porcentaje_varones_mujeres(Persona, PorcentajeVarones, PorcentajeMujeres) :-
    cantidad_hijos_y_genero(Persona, Total, Varones, Mujeres),
    PorcentajeVarones is (Varones / Total) * 100,
    PorcentajeMujeres is (Mujeres / Total) * 100.
